...
Default port is 8000. SSI is enabled if file name ends with '.shtml'.
...
def _expand_shtml(doc_root, fs):
    ...
    return '<!-- AUTO GENERATED HTML: DO NOT MODIFY MANUALLY -->\n' + ''.join(slines)

def _generate(doc_root, fs):
    '''
...
def generate(doc_root, rel_path):
    '''
    ...
    for f in os.listdir(os.path.join(doc_root, rel_path)):
        if os.path.isdir(os.path.join(doc_root, rel_path, f)):
            ...
            if f.startswith('.') or f in DIR_IGNORE:
                ...
                generate(doc_root, next_rel_path)
        else:
            if f.endswith('.shtml'):
                _generate(doc_root, os.path.join(rel_path, f))
